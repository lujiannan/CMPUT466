...
def train(X_train, y_train, X_val, y_val, lambdas=(3, 1, 0.3, 0.1, 0.03, 0.01)):
    ...
    for lam in lambdas:
        for epoch in range(MaxIter):
            ...
            for b in range(int(np.ceil(N_train/batch_size))):
                ...
                w = w - alpha * (1/len(y_batch) * (X_batch.T).dot(y_hat_batch-y_batch) + lam*w)
            ...
            if risk == np.min(risks_val):
                ...
                risk_best = risk
                w_best = w
        # parameters under best lambda
        if (risk_best < risk_opt):
            losses_train_opt = losses_train
            risks_val_opt = risks_val
            lambda_opt = lam
            risk_opt = risk_best
            epoch_opt = epoch_best
            w_opt = w_best
        # resetting variables
        w = np.zeros([X_train.shape[1], 1])
        losses_train = []
    ...
    return losses_train_opt, risks_val_opt, epoch_opt, risk_opt, w_opt, lambda_opt
...
losses_train, risks_val, epoch_best, risk_best, w_best, lambda_opt = train(X_train, y_train, X_val, y_val)
