...
def train(X_train, y_train, X_val, y_val, lam=0.01):
...
alphas = (0.05,0.01,0.005,0.001,0.0005)      # learning rate
...
for alpha in alphas:
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    # plt.savefig("q2b_loss_alpha=" + str(alpha) + ".jpg")

    plt.figure()
    plt.plot(range(MaxIter), risks_val)
    plt.xlabel("Epoch")
    plt.ylabel("Risk")
    # plt.savefig("q2b_risk_alpha=" + str(alpha) + ".jpg")
